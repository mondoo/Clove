...
//Temp to get regular PCF working
static const float3 poissonDisk3D[20] = {
...
float2 getAverageBlockerDistance(TextureCubeArray tex, SamplerState state, float3 fragToLight, float farPlane, int lightIndex, float offset){
    ...
    for(int i = 0; i < poissonDiskSamples; ++i){
        const float3 sampleLocation = fragToLight + poissonDisk3D[i] * float3(offset, offset, 1.0f / 6.0f);
...
float GenerateShadow_PCSS(TextureCubeArray tex, SamplerState state, float3 fragToLight, float farPlane, int lightIndex, float shadowOffsetBias, float diskRadius){
    ...
    float averageBlockerDistance = getAverageBlockerDistance(tex, state, fragToLight, farPlane, lightIndex, offset);
