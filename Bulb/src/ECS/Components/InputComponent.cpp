...
namespace blb::ecs {
	...
	void InputComponent::unbind(clv::Key key, BindingId id) {
	...
	void InputComponent::unbind(clv::MouseButton button, BindingId id) {
		std::unordered_map<BindingId, size_t>& idToIndexMap = buttonIdToIndexMap[button];
		if(auto iter = idToIndexMap.find(id); iter != idToIndexMap.end()) {
			std::vector<MouseBindingFunction>& buttonVector = mouseButtonBindings[button];

			const size_t index		= iter->second;
			const size_t lastIndex	= buttonVector.size() - 1;

			if(index < lastIndex) {
				buttonVector[index] = buttonVector.back();
			}
