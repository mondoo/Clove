...
namespace blb::rnd {
    ...
    Animator::Animator(const Animator& other) = default;

    Animator::Animator(Animator&& other) noexcept = default;

    Animator& Animator::operator=(const Animator& other) = default;

    Animator& Animator::operator=(Animator&& other) noexcept = default;

    Animator::~Animator() = default;

    std::array<clv::mth::mat4f, MAX_JOINTS> Animator::update(const clv::utl::DeltaTime deltaTime) {
    ...
    std::pair<const AnimationPose&, const AnimationPose&> Animator::getPrevNextPose(float animationTime) {
        size_t prevIndex = 0;
        size_t nextIndex = 0;
