...
constexpr std::uint32_t crc32(const char* str){
	auto val = crc32<idx - 1>(str);
	return (val >> 8) ^ crc_table[(val ^ str[idx]) & 0x000000FF];
}

// This is the stop-recursion function
template<>
constexpr std::uint32_t crc32<std::size_t(-1)>(const char* str){
	return 0xFFFFFFFF;
}
...
#define HASH_CLASS(x) COMPILE_TIME_CRC32_STR(GENERATE_STRING(x))