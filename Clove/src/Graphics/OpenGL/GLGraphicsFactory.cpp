...
namespace clv::gfx::ogl {
    ...
    std::shared_ptr<Shader> GLGraphicsFactory::createShader(ShaderDescriptor descriptor, std::string_view pathToShader) {
        return std::make_shared<GLShader>(shared_from_this(), std::move(descriptor), pathToShader);
    }

    std::shared_ptr<Shader> GLGraphicsFactory::createShader(ShaderDescriptor descriptor, std::span<const std::byte> sourceBytes) {
        return std::make_shared<GLShader>(shared_from_this(), std::move(descriptor), std::move(sourceBytes));
    }
