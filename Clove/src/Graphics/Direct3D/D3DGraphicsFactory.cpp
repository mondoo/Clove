...
namespace clv::gfx::d3d {
	D3DGraphicsFactory::D3DGraphicsFactory(Microsoft::WRL::ComPtr<ID3D11Device> d3dDevice, Microsoft::WRL::ComPtr<ID3D11DeviceContext> d3dDeviceContext)
		...
		, d3dDeviceContext(std::move(d3dDeviceContext)) {
	}

	D3DGraphicsFactory::~D3DGraphicsFactory() = default;

	std::shared_ptr<CommandBuffer> D3DGraphicsFactory::createCommandBuffer() {
		return std::make_shared<D3DCommandBuffer>(shared_from_this(), *d3dDevice.Get());
	}
	...
	std::shared_ptr<Shader> D3DGraphicsFactory::createShader(ShaderDescriptor descriptor, std::string_view pathToShader) {
		return std::make_shared<D3DShader>(shared_from_this(), *d3dDevice.Get(), std::move(descriptor), pathToShader);
	}

	std::shared_ptr<Shader> D3DGraphicsFactory::createShader(ShaderDescriptor descriptor, std::span<const std::byte> sourceBytes) {
        return std::make_shared<D3DShader>(shared_from_this(), *d3dDevice.Get(), std::move(descriptor), std::move(sourceBytes));
	}

	std::shared_ptr<Surface> D3DGraphicsFactory::createSurface(void* windowData) {
		auto surface = std::make_shared<D3DSurface>(shared_from_this(), *d3dDevice.Get(), windowData);
		surface->onDeviceRemoved.bind(&D3DGraphicsFactory::onDeviceRemoved, this);
		return surface;
	}

#if CLV_IMGUI_COMPATIBLE
	const Microsoft::WRL::ComPtr<ID3D11Device>& D3DGraphicsFactory::getDevice() const {
		return d3dDevice;
	}

	const Microsoft::WRL::ComPtr<ID3D11DeviceContext>& D3DGraphicsFactory::getContext() const {
