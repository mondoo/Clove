...
namespace clv::gfx::BindableFactory{
	...
	std::shared_ptr<Texture>				createTexture(const std::string& filePath, uint32 bindingPoint, TextureUsage usageType = TextureUsage::Default);
	...
	std::shared_ptr<Texture>				createTexture(int32 width, int32 height, uint32 bindingPoint, TextureUsage usageType = TextureUsage::Default);
