...
namespace clv{
	namespace gfx{
		...
		template<typename T>
		inline DX11ConstantBuffer<T>::DX11ConstantBuffer(const T& data, Renderer* renderer){
		...
		template<typename T>
		inline DX11ConstantBuffer<T>::DX11ConstantBuffer(Renderer* renderer){
			if(DX11Renderer* dxrenderer = dynamic_cast<DX11Renderer*>(renderer)){
				DX11_INFO_PROVIDER(dxrenderer);
		...
		template<typename T>
		inline void VertexConstantBuffer<T>::bind(Renderer* renderer){
			if(DX11Renderer* dxrenderer = dynamic_cast<DX11Renderer*>(renderer)){
		...
		template<typename T>
		inline void VertexConstantBuffer<T>::unbind(){
		}

		template<typename T>
		inline void PixelConstantBuffer<T>::bind(Renderer * renderer){
			if(DX11Renderer* dxrenderer = dynamic_cast<DX11Renderer*>(renderer)){
		...
		template<typename T>
		inline void PixelConstantBuffer<T>::unbind(){
		}
	}
}