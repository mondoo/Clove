...
namespace clv {
    ...
    std::string WindowsException::translateErrorCode(HRESULT hr) noexcept {
        ...
        if(sizeNeeded <= 0) {
            GARLIC_LOG(garlicLogContext, garlic::LogLevel::Error, "Was unable to convert wide char to multi byte");
            GARLIC_LOG(garlicLogContext, garlic::LogLevel::Error, GetLastError());
        } else {
