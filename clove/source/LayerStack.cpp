...
namespace garlic::clove {
    ...
    LayerStack::~LayerStack() {
        CLOVE_LOG(LOG_CATEGORY_CLOVE, LogLevel::Trace, "Deconstructing layer stack...");
        for(auto &layer : layers) {
            CLOVE_LOG(LOG_CATEGORY_CLOVE, LogLevel::Debug, "{0} detached", layer->getName());
            layer->onDetach();
        }
    }
    ...
    void LayerStack::popLayer(std::shared_ptr<Layer> const &layer) {
        if(auto it = std::find(layers.begin(), layers.end(), layer); it != layers.end()) {
            CLOVE_LOG(LOG_CATEGORY_CLOVE, LogLevel::Debug, "Popped layer: {0}", (*it)->getName());
            (*it)->onDetach();
            layers.erase(it);
            --layerInsertIndex;
        }
    ...
    void LayerStack::pushOverlay(std::shared_ptr<Layer> overlay) {
        CLOVE_LOG(LOG_CATEGORY_CLOVE, LogLevel::Debug, "Attached overlay: {0}", overlay->getName());
        overlay->onAttach();
    ...
    void LayerStack::popOverlay(std::shared_ptr<Layer> const &overlay) {
        if(auto it = std::find(layers.begin(), layers.end(), overlay); it != layers.end()) {
            CLOVE_LOG(LOG_CATEGORY_CLOVE, LogLevel::Debug, "Popped overlay: {0}", (*it)->getName());
            (*it)->onDetach();
            layers.erase(it);
        }
    }
