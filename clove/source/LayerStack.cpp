...
namespace garlic::clove {
    ...
    LayerStack::~LayerStack() {
        ...
        for(auto &layer : layers) {
            CLOVE_LOG(LOG_CATEGORY_CLOVE, LogLevel::Trace, "{0} detached", layer->getName());
            layer->onDetach();
        }
    }
    ...
    void LayerStack::pushOverlay(std::shared_ptr<Layer> overlay) {
        CLOVE_LOG(LOG_CATEGORY_CLOVE, LogLevel::Trace, "Attached overlay: {0}", overlay->getName());
        overlay->onAttach();
    ...
    void LayerStack::popOverlay(std::shared_ptr<Layer> const &overlay) {
        if(auto it = std::find(layers.begin(), layers.end(), overlay); it != layers.end()) {
            CLOVE_LOG(LOG_CATEGORY_CLOVE, LogLevel::Trace, "Popped overlay: {0}", (*it)->getName());
            (*it)->onDetach();
            layers.erase(it);
        }
    }
