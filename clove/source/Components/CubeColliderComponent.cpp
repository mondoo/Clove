...
namespace garlic::clove {
    CubeColliderComponent::CubeColliderComponent()
        : CubeColliderComponent(vec3f{ 1.0f }) {
    }

    CubeColliderComponent::CubeColliderComponent(vec3f cubeSize)
    ...
    void CubeColliderComponent::setCubeSize(vec3f cubeSize) {
        collisionShape->setLocalScaling(btVector3{ cubeSize.x, cubeSize.y, cubeSize.z });
        this->cubeSize = std::move(cubeSize);
    }

    vec3f const &CubeColliderComponent::getCubeSize() const {
        return cubeSize;
    }
