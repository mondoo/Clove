...
namespace clove::reflection {
    ...
    std::optional<AttributeType> getAttribute(TypeInfo<ClassType> const &typeInfo) {
        if constexpr(internal::HasType<AttributeType, std::decay_t<decltype(typeInfo.attributes)>>::value) {
            return std::get<AttributeType>(typeInfo.attributes);
        } else {
            return {};
        }
    }
    ...
    std::optional<AttributeType> getAttribute(MemberType const &member) {
        if constexpr(internal::HasType<AttributeType, std::decay_t<decltype(member.attributes)>>::value) {
            return std::get<AttributeType>(member.attributes);
        } else {
            return {};
        }
    }
