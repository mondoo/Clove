...
namespace garlic::clove::ShaderTranspiler {
    static EShLanguage getEShStage(Shader::Stage stage) {
        switch(stage) {
            ...
            default:
                GARLIC_LOG(garlicLogContext, LogLevel::Error, "Unsupported shader stage {0}", GARLIC_FUNCTION_NAME);
                return EShLanguage::EShLangVertex;
        }
    }
    ...
    std::string transpileFromSource(std::string_view source, Shader::Stage stage, ShaderType outputType) {
        ...
        if(strlen(log) > 0) {
            GARLIC_LOG(garlicLogContext, LogLevel::Error, "Error compiling shader: {0}", log);
            return "";
        }
