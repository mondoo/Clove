...
namespace garlic::clove {
    ...
    Expected<std::unique_ptr<PresentQueue>, std::string> VKGraphicsFactory::createPresentQueue() {
        if(queueFamilyIndices.presentFamily.has_value()) {
            std::unique_ptr<PresentQueue> queue = std::make_unique<VKPresentQueue>(devicePtr, *queueFamilyIndices.presentFamily);
            return queue;
        } else {
            return Unexpected<std::string>("Presentation queue not available. GraphicsDevice is likely headless");
        }
    }
    ...
    Expected<std::unique_ptr<Swapchain>, std::string> VKGraphicsFactory::createSwapChain(Swapchain::Descriptor descriptor) {
        if(swapchainSupportDetails.has_value()) {
            ...
            return Unexpected<std::string>("Swapchain is not available. GraphicsDevice is likely headless");
        }
    }
