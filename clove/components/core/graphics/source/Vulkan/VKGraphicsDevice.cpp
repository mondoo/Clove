...
static VKAPI_ATTR VkBool32 VKAPI_CALL debugCallback(
    ...
    if((messageSeverity & VK_DEBUG_UTILS_MESSAGE_SEVERITY_INFO_BIT_EXT) != 0) {
        GARLIC_LOG(LOG_CATEGORY_GARLIC, garlic::clove::LogLevel::Trace, pCallbackData->pMessage);
    } else if((messageSeverity & VK_DEBUG_UTILS_MESSAGE_SEVERITY_WARNING_BIT_EXT) != 0) {
        GARLIC_LOG(LOG_CATEGORY_GARLIC, garlic::clove::LogLevel::Warning, pCallbackData->pMessage);
    } else if((messageSeverity & VK_DEBUG_UTILS_MESSAGE_SEVERITY_ERROR_BIT_EXT) != 0) {
        GARLIC_LOG(LOG_CATEGORY_GARLIC, garlic::clove::LogLevel::Error, pCallbackData->pMessage);
    }
...
namespace garlic::clove {
    ...
    VKGraphicsDevice::VKGraphicsDevice(std::any nativeWindow) {
        ...
        if(!checkValidationLayerSupport(validationLayers)) {
            GARLIC_LOG(LOG_CATEGORY_GARLIC, LogLevel::Warning, "Vulkan validation layers are not supported on this device. Unable to provide debugging infomation");
        }
#endif
        ...
        VkDebugUtilsMessengerEXT debugMessenger{ VK_NULL_HANDLE };
        {
            ...
            if(vkCreateInstance(&createInfo, nullptr, &instance) != VK_SUCCESS) {
                GARLIC_LOG(LOG_CATEGORY_GARLIC, LogLevel::Error, "Failed to create VK instance");
                return;
            }

#if GARLIC_DEBUG
            if(createDebugUtilsMessengerEXT(instance, &debugMessengerCreateInfo, nullptr, &debugMessenger) != VK_SUCCESS) {
                GARLIC_LOG(LOG_CATEGORY_GARLIC, LogLevel::Error, "Failed to create vk debug message callback");
                return;
            }
#endif
        }
        ...
        if(nativeWindow.has_value()) {
            ...
            if(vkCreateWin32SurfaceKHR(instance, &createInfo, nullptr, &surface) != VK_SUCCESS) {
                GARLIC_LOG(LOG_CATEGORY_GARLIC, LogLevel::Error, "Failed to create Vulkan surface");
                return;
            }
#elif GARLIC_PLATFORM_MACOS
        ...
        VkPhysicalDevice physicalDevice;
        {
            ...
            if(deviceCount == 0) {
                GARLIC_LOG(LOG_CATEGORY_GARLIC, LogLevel::Error, "failed to find GPUs with Vulkan support!");
                return;
            }
            ...
            if(physicalDevice == VK_NULL_HANDLE) {
                GARLIC_LOG(LOG_CATEGORY_GARLIC, LogLevel::Error, "failed to find a suitable GPU!");
                return;
            } else {
                VkPhysicalDeviceProperties devicePoperties;
                vkGetPhysicalDeviceProperties(physicalDevice, &devicePoperties);

                GARLIC_LOG(LOG_CATEGORY_GARLIC, LogLevel::Info, "Vulkan capable physical device found");
                GARLIC_LOG(LOG_CATEGORY_GARLIC, LogLevel::Info, "\tDevice:\t{0}", devicePoperties.deviceName);
                GARLIC_LOG(LOG_CATEGORY_GARLIC, LogLevel::Info, "\tDriver:\t{0}.{1}.{2}", VK_VERSION_MAJOR(devicePoperties.driverVersion), VK_VERSION_MINOR(devicePoperties.driverVersion), VK_VERSION_PATCH(devicePoperties.driverVersion));
                GARLIC_LOG(LOG_CATEGORY_GARLIC, LogLevel::Info, "\tAPI:\t{0}.{1}.{2}", VK_VERSION_MAJOR(devicePoperties.apiVersion), VK_VERSION_MINOR(devicePoperties.apiVersion), VK_VERSION_PATCH(devicePoperties.apiVersion));
            }
        }
        ...
        VkDevice logicalDevice;
        {
            ...
            if(vkCreateDevice(physicalDevice, &createInfo, nullptr, &logicalDevice) != VK_SUCCESS) {
                GARLIC_LOG(LOG_CATEGORY_GARLIC, LogLevel::Error, "Failed to create logical device");
                return;
            }
        }
