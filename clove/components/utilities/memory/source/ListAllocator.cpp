    ...
    ListAllocator::~ListAllocator() {
        if(freeMemory) {
            ...
            if(allocations > frees) {
                GARLIC_LOG(garlicLogContext, LogLevel::Warning, "List Allocator destructed with active memory. Block will be freed but destructors will not be called on occupying elements");
            }
#endif
            ::free(rawList);
        }
